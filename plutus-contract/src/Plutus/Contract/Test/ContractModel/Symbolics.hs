...
import Data.Maybe
import Ledger.Tx.CardanoAPI (toCardanoAssetId, toCardanoValue)
...
instance SymValueLike Value where
  toSymValue = SymValue mempty . fromRight mempty . toCardanoValue
...
instance TokenLike AssetClass where
  symAssetClassValueOf (SymValue _ v) t = either (const 0) (\t' -> case C.selectAsset v t' of C.Quantity i -> i) $ toCardanoAssetId t
  symAssetClassValue t i = toSymValue $ assetClassValue t i

instance TokenLike C.AssetId where
  symAssetClassValueOf (SymValue _ v) t = case C.selectAsset v t of C.Quantity i -> i
  symAssetClassValue t i = toSymValue (C.valueFromList [(t, C.Quantity i)])
