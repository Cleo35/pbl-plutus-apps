...
``contributions`` field from it, and then combines all the :hsobj:`Plutus.Script.Utils.Value`
...
.. note::
   ...
   We can't use ``(>=)`` to compare :hsobj:`Plutus.Script.Utils.Value`; there is no
   ``Ord`` instance. That is because some :hsobj:`Plutus.Script.Utils.Value` are incomparable,
...
Plutus :hsobj:`Plutus.Script.Utils.Value` with an extra type wrapper.
...
a :hsobj:`Plutus.Script.Utils.Value` for sale, creating an auction UTXO containing the value,
...
Now ``prop_Auction`` fails!
 ...
 .. note::
    ...
    The balance change is actually a :hsobj:`Plutus.Contract.Test.ContractModel.Symbolics.SymValue`, not a :hsobj:`Plutus.Script.Utils.Value`,
    but as you can see it *contains* a :hsobj:`Plutus.Script.Utils.Value`, which is all we care
    about right now.
