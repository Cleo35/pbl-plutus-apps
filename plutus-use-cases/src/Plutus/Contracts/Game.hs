...
import Plutus.Contract (AsContractError, Contract, Endpoint, Promise, collectFromScript, endpoint, fundsAtAddressGeq,
                        logInfo, mkTxConstraints, selectList, type (.\/), utxosAt, yieldUnbalancedTx)
...
lock = endpoint @"lock" $ \LockArgs { lockArgsGameParam, lockArgsSecret, lockArgsValue } -> do
    ...
    logInfo @Haskell.String $ Haskell.show $ Constraints.adjustUnbalancedTx unbalancedTx
...
guess = endpoint @"guess" $ \GuessArgs { guessArgsGameParam, guessArgsSecret } -> do
    ...
    logInfo @Haskell.String $ "guess!!!!!!!"
    logInfo @Haskell.String $ Haskell.show $ gameAddress guessArgsGameParam
    myutxos <- utxosAt (gameAddress guessArgsGameParam)
    logInfo @Haskell.String $ Haskell.show myutxos
    ...
    logInfo @Haskell.String "Yielding transaction to guess the secret word"
    unbalancedTx <- mkTxConstraints lookups tx
