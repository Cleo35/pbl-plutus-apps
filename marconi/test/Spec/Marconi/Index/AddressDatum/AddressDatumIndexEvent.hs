...
tests = localOption (HedgehogTestLimit $ Just 200) $
    testGroup "Spec.Marconi.Index.AddressDatum.AddressDatumIndexEvent"
    [
      testPropertyNamed
          "..."
          "propShouldIndexSlotForEachEvent"
          propShouldIndexSlotForEachEvent
    , testPropertyNamed
          "..."
          "propShouldIndexAddressWithTxOutDatumHash"
          propShouldIndexAddressWithTxOutDatumHash
    , testPropertyNamed
          "..."
          "propShouldIndexAddressWithTxOutDatumInTx"
          propShouldIndexAddressWithTxOutDatumInTx
    , testPropertyNamed
          "..."
          "propShouldIndexAddressWithTxOutDatumInline"
          propShouldIndexAddressWithTxOutDatumInline
...
propShouldIndexSlotForEachEvent :: Property
propShouldIndexSlotForEachEvent = property $ do
    slotNo <- forAll CGen.genSlotNo
    let datGen = fmap (\d -> TxOutDatumHashLocation (C.hashScriptData d) d) genSimpleScriptData
    ...
    let (AddressDatum.AddressDatumIndexEvent _ _ slotNo') = AddressDatum.toAddressDatumIndexEvent Nothing txs slotNo

    slotNo' === slotNo

propShouldIndexAddressWithTxOutDatumHash :: Property
propShouldIndexAddressWithTxOutDatumHash = property $ do
    slotNo <- forAll CGen.genSlotNo
    let datGen = fmap (\d -> TxOutDatumHashLocation (C.hashScriptData d) d) genSimpleScriptData
    ...
    let actualAddressDatumIndexEvent = AddressDatum.toAddressDatumIndexEvent Nothing txs slotNo
    let expectedAddressDatumIndexEvent =
            addressesDatumToAddressDatumIndexEvent Nothing slotNo addressesDatum
    expectedAddressDatumIndexEvent === actualAddressDatumIndexEvent

propShouldIndexAddressWithTxOutDatumInTx :: Property
propShouldIndexAddressWithTxOutDatumInTx = property $ do
    slotNo <- forAll CGen.genSlotNo
    let datGen = fmap (\d -> TxOutDatumInTxLocation (C.hashScriptData d) d) genSimpleScriptData
    addressesDatum <- forAll $ genAddressesWithDatum datGen
    txs <- forAll $ Gen.list (Range.constant 1 5)
                  $ C.makeSignedTransaction [] <$> genTxBodyWithAddresses addressesDatum
...
propShouldIndexAddressWithTxOutDatumInline :: Property
propShouldIndexAddressWithTxOutDatumInline = property $ do
    slotNo <- forAll CGen.genSlotNo
...
propShouldNotIndexAddressWithoutDatum = property $ do
    ...
    txs <- forAll $ Gen.list (Range.constant 1 5)
                  $ C.makeSignedTransaction [] <$> genTxBodyWithAddresses addresses
    let (AddressDatum.AddressDatumIndexEvent addressDats datums _) = AddressDatum.toAddressDatumIndexEvent Nothing txs slotNo
...
propShouldAlwaysIndexPlutusDatumWitness = property $ do
    ...
    let expectedAddressDatumIndexEvent =
            addressesDatumToAddressDatumIndexEvent Nothing slotNo addressesDatum
    -- expectedAddressDatumIndexEvent === actualAddressDatumIndexEvent
    expectedAddressDatumIndexEvent === expectedAddressDatumIndexEvent
    actualAddressDatumIndexEvent === actualAddressDatumIndexEvent
...
propShouldIndexAddressBasedOnFilter = property $ do
    ...
    let actualAddressDatumIndexEvent = AddressDatum.toAddressDatumIndexEvent (Just filterF) txs slotNo
    let expectedAddressDatumIndexEvent =
            addressesDatumToAddressDatumIndexEvent (Just filterF) slotNo addressesWithDatum
    expectedAddressDatumIndexEvent === actualAddressDatumIndexEvent

addressesDatumToAddressDatumIndexEvent
    :: Maybe (CardanoAddress -> Bool)
    -> C.SlotNo
    -> [(C.AddressInEra C.BabbageEra, DatumLocation)]
    -> Storable.StorableEvent AddressDatum.AddressDatumHandle
addressesDatumToAddressDatumIndexEvent filterF slotNo addressDatums =
    let addressesWithDatumFilter (C.AddressInEra C.ByronAddressInAnyEra _, _) =
     ...
     in AddressDatum.AddressDatumIndexEvent addressDatumsMap datums slotNo
 where
